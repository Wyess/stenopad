...
rm output.mp4  
ffmpeg -r 24 -i kasaga_nai_%05d.svg -vf "pad=ceil(iw/2)*2+10:ceil(ih/2)*2+10" -pix_fmt yuv420p output.mp4
#ffmpeg -f lavfi -i color=c=white:s=1080x1920:r=24 -i kasaga_nai_%05d.svg -filter_complex "[0:v][1:v]overlay=shortest=1,format=yuv420p" output.mp4
rm ./*.svg
